...
async function debugDepthCalc() {
    ...
    console.log("🔍 Debugging depth calculation step by step...");
    
    const results = await page.evaluate(() => {
        const ocean = window.oceanInstance;
        if (!ocean) return { error: "No ocean instance" };
        
        // Test continental shelf position
        const x = -45000, z = 0;
    ...
    console.log("Depth calculation breakdown:");
    console.log(JSON.stringify(results, null, 2));
    
    await browser.close();
}

debugDepthCalc().catch(console.error);