...
class SimpleTerrain {
    ...
    createEmergencyTerrain() {
        ...
        // Expanded terrain size to 50x50km for larger submarine operation area
        const width = 50000;          // 50km width
        const height = 50000;         // 50km height  
        const widthSegments = 16;     // Increased segments for larger terrain
        const heightSegments = 16;    // Increased segments for larger terrain
        ...
        for (let i = 0; i < vertices.length; i += 3) {
            ...
            // Height variations scaled for 50km terrain
            const heightVariation1 = Math.sin(x * 0.0002) * 120; // Large rolling hills (longer wavelength)
            const heightVariation2 = Math.sin(z * 0.0003) * 80;  // Cross-direction valleys
            const heightVariation3 = Math.sin(x * 0.001 + z * 0.0008) * 40; // Fine detail layer
            const totalVariation = heightVariation1 + heightVariation2 + heightVariation3;
            const newHeight = -150 + totalVariation; // Deeper base depth for larger terrain
        ...
        console.log(`✅ EXPANDED TERRAIN: 50x50km terrain in ${this.wireframeMode ? 'WIREFRAME' : 'SOLID'} mode`);
        console.log('📊 Mesh info: position =', mesh.position, ', visible =', mesh.visible);
        console.log('📊 Terrain size: 50km × 50km (25x larger than before)');
        console.log('📊 Height range: approximately -390m to +90m (480m total variation)');
        console.log('📊 Resolution: 16×16 segments for smooth large-scale terrain');
        
        // Add cyan reference plane above the terrain for comparison  
        const testGeometry = new THREE.PlaneGeometry(5000, 5000); // 5km reference plane
        ...
        testMesh.position.set(0, -50, 0); // Above the tallest terrain parts
        this.terrainGroup.add(testMesh);
        console.log('✅ Cyan reference plane (5km×5km) added at Y=-50 (above terrain)');
